...
- [ ] **Payment Submission**: Update `frontend/user_device/user.js` (representing the EV Owner) to send the copied ASCON string (VFID), VMID, PIN, and Amount via `POST /api/user/authorize`.
...
- [ ] **Grid Authentication**: Ensure the grid correctly decrypts the ASCON string back to the FID, and verifies the EV Owner's VMID, hashed PIN, and account balance.
- [ ] **Blockchain Mining**: Once validated, ensure the transaction is hashed (SHA-3) and mined into the Grid's central blockchain.
- [ ] **Franchise/Grid UI Updates**: Connect `frontend/grid_dashboard/grid.js` to `GET /api/grid/ledger` so it live-updates with pass/fail results and the newly mined blocks.
- [ ] **Handle Edge Cases**: Add UI feedback for Insufficient Balance, Invalid PIN, or Hardware Dispense Failures (Refunds).